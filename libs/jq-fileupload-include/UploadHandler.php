...
class UploadHandler
{
    ...
    function __construct($options = null, $initialize = true, $error_messages = null) {
        $this->options = array(
            ...
            'accept_file_types' => '/.+\.(txt|csv|dat)$/i',
    ...
    protected function set_additional_file_properties($file) {
        ...
        // ALPER edit:
        // hack to allow sending back a preview of what the first couple lines 
        // of the file looks like (first three lines, truncated to 70 chars each)
        $file_path = $this->get_upload_path($file->name);
        $handle = fopen($file_path, "r");
        $file->preview = '';
        
        $counter = 0; $lineLimit = 5;
        if ($handle) {
            while(($line = fgets($handle)) !== false) {
                if (strlen($line) > 70) {
                    $file->preview .= htmlentities(substr($line, 0, 70)) . '...' . PHP_EOL;
                } else {
                    $file->preview .= htmlentities($line);
                }
